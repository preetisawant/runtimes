...
### Trace und Speicherauszug in der Konsole von {{site.data.keyword.Bluemix_notm}} verwenden (nicht mehr verwendet)
...
Verwenden Sie den folgenden Befehl, um einen Thread- und Heapspeicherauszug mit dem Feature SSH über CF CLI auszulösen:
...
Verschiedene Speicherauszugsdateien werden standardmäßig im Verzeichnis `dumps` des Anwendungscontainers gespeichert. Verwenden Sie die Cloud Foundry-CLI `cf ssh`, um die Speicherauszugsdateien anzuzeigen und herunterzuladen.
