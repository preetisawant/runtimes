...
App Management는 {{site.data.keyword.Bluemix}}에서 Liberty 및 Node.js 애플리케이션에 사용될 수 있는 개발 및 디버깅 유틸리티 세트입니다.
{:shortdesc}
...
* Node.js 애플리케이션의 경우, 대부분의 App Management 유틸리티는 `CF CLI (-c)` 또는 `manifest.yml` 파일(명령)에서 **시작** 명령을 설정하는 경우에는 작동되지 않습니다. 이러한 메소드는 빌드팩 대체이며, Node.js 애플리케이션 시작에 대한 반대 패턴입니다. 최상의 결과를 얻으려면 `package.json` 파일 또는 `Procfile`에서 **시작** 명령을 설정하십시오. 

### Liberty 및 Node.js 유틸리티
...
*hc* 유틸리티를 *noproxy*와 함께 사용할 수 있습니다. Health Center를 *noproxy*와 함께 사용하려면 우선 `cf ssh` 명령을 사용하여 포트 전달을 설정하십시오. 예를 들어, 다음과 같습니다.
...
Diego에서 `cf ssh` 명령을 통해 대화식 쉘을 제공하므로, *shell* 유틸리티는 DEA에서 실행 중인 애플리케이션에만 유용합니다.
{: .tip}
...
*debug* 유틸리티를 사용하려면 [Eclipse Tools for {{site.data.keyword.Bluemix_notm}}](https://console.bluemix.net/docs/manageapps/eclipsetools/eclipsetools.html)를 설치해야 합니다. 

*debug* 유틸리티는 Liberty 애플리케이션을 디버그 모드로 설정하며, IBM Eclipse Tools for {{site.data.keyword.Bluemix_notm}} 등의 클라이언트가 애플리케이션에서 [원격 디버깅](https://console.bluemix.net/docs/manageapps/eclipsetools/eclipsetools.html#remotedebug) 세션을 설정할 수 있도록 합니다. 

**중요사항:** *debug* 유틸리티는 *proxy*를 시작합니다. 

*debug* 유틸리티를 *noproxy*와 함께 사용할 수 있습니다. debug 유틸리티를 *noproxy*와 함께 사용하려면 우선 `cf ssh` 명령을 사용하여 포트 전달을 설정하십시오. 

다음의 코드 스니펫은 `cf ssh` 명령 형식의 예를 보여줍니다. 
...
다음으로, Eclipse에서 연결하려면 *원격 Java 구성*을 사용하여 호스트를 `127.0.0.1`로 지정하고 포트를 `7777`로 지정하십시오. 

#### jmx
{: #jmx}

*jmx* 유틸리티는 JMX REST 커넥터를 사용하여 원격 JMX 클라이언트가 {{site.data.keyword.Bluemix_notm}} 사용자 신임 정보를 사용하여 애플리케이션을 관리할 수 있도록 허용합니다. 
...
*localjmx* 유틸리티는 Diego 셀에서 실행 중인 애플리케이션에만 적용됩니다. *localjmx*를 사용하려면 우선 `cf ssh` 명령을 사용하여 포트 전달을 설정하십시오. 예를 들어, 다음과 같습니다.
