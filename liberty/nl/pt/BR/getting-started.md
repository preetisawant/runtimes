...
A implementação de seu aplicativo pode levar alguns minutos. Quando a implementação for concluída, uma mensagem mostrará que seu app está em execução. Visualize seu app na URL listada na saída do comando push com "/GetStartedJava" anexado no final ou visualize o status de implementação do app e a URL executando o comando a seguir:
...
Também é possível acessar a [Lista de recursos](https://cloud.ibm.com/resources) do {{site.data.keyword.Bluemix_notm}} para visualizar o seu aplicativo. Se você clicar em **Visitar a URL do app**, lembre-se de anexar "/GetStartedJava" à URL.

É possível solucionar problemas de erros no processo de implementação usando o comando `ibmcloud cf logs GetStartedJava --recent`.
{: tip}  
