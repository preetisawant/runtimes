...
자동 서비스 구성을 사용하지 않으려면 services_autoconfig_excludes 환경 변수를 사용하십시오. 이 환경 변수를 manifest.yml에 포함하거나 cf 클라이언트를 사용하여 설정합니다.
...
**중요**: ```<service_type_specification>```에 공백을 사용할 수 없습니다. 공백은 다수의 ```<service_type_specification>``` 인스턴스를 구분하는 용도로만 유일하게 허용됩니다. 
